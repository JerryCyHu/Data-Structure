  ...
  //
      while (i > 0 && i < (int)size()) {
          ...
          if (m_heap[i]->getPriorityValue() < m_heap[p]->getPriorityValue()) {
              locations[m_heap[i]] = locations[m_heap[p]];
              locations[m_heap[p]] = i;
              T tmp = m_heap[p]; // value and other non-pointer member vars
              m_heap[p] = m_heap[i];
              m_heap[i] = tmp;
              i = p;
          } else
...
// Choose the child to compare against
        if (has_right_child(i) && m_heap[get_right_child(i)]->getPriorityValue() < m_heap[get_left_child(i)]->getPriorityValue())
            ...
            c = get_left_child(i);
        if (m_heap[c]->getPriorityValue() < m_heap[i]->getPriorityValue()) {
            locations[m_heap[i]] = locations[m_heap[c]];
            locations[m_heap[c]] = i;
            T tmp = m_heap[c]; // value and other non-pointer member vars
            m_heap[c] = m_heap[i];
            m_heap[i] = tmp;
            i = c;
        }
