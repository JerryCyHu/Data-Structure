...
public:
  ...
  // FIND, INSERT & ERASE
  iterator find(const T& key_value) { return helperFind(key_value, root_); }
...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    ...
    TreeNode<T>* copiedTree;
    helperCopy(copiedTree, old_root);
      return copiedTree;
  }

  void helperCopy(TreeNode<T>*& p, TreeNode<T>* old_p){
      if (!old_p){
          return;
      } else{
          p = new TreeNode<T>();
          p->value = old_p->value;
          helperCopy(p->left, old_p->left);
          helperCopy(p->right, old_p->right);
      }
  }
  ...
  iterator helperFind(const T& key_value, TreeNode<T>* p){
      TreeNode<T>* tree = p;
      while (tree){
          if (tree->value<key_value)
              tree = tree->right;
          else if (tree->value>key_value)
              tree = tree->left;
          else
