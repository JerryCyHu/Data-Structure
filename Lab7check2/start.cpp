...
int countingSteps(int x, int y, vector<Point>& blockedPoints){
    ...
    if (x == 0 && y == 0){
        ...
        for(int i = 0; i<blockedPoints.size(); i++){
            if (blockedPoints[i].x == x && blockedPoints[i].y == y){
                valid = false;
                break;
            }
        }
        if (valid){
        countingSteps(x, y-1,blockedPoints);
        }
    } else if(x > 0 && y == 0){
        bool valid = true;
        for(int i = 0; i<blockedPoints.size(); i++){
        ...
        if (valid) {
            countingSteps(x - 1, y, blockedPoints);
        }
    } else{
        bool valid = true;
        for(int i = 0; i<blockedPoints.size(); i++){
...
void makeBlockPointVector(istream& istr, vector<Point>& blockedPoints){
    // Read the x y locations into a list of Points.
    int x, y;
    while ((istr >> x >> y) && ! (x==0 && y==0)) {
        Point p = Point(x,y);
        blockedPoints.push_back(Point(x,y));
    }
}

bool findAPath(vector<Point>& path, vector<Point>& blockedPoints, int x, int y){
    if(x == 0&&y == 0){
        return true;
    }
    vector<Point> path_copy;
    for (Point p:path) {
        path_copy.push_back(p);
    }
    bool pathX = false, pathY = false;
    if(x>0) {
        pathX = findAPath(path, blockedPoints, x - 1, y);
    }
    if(y>0) {
        pathY = findAPath(path, blockedPoints, x, y - 1);
    }
    bool valid = true;
    for(int i = 0; i<blockedPoints.size(); i++){
        if (blockedPoints[i].x == x && blockedPoints[i].y == y){
    ...
    if(valid == false){
        return false;
    }else if (pathX == false&&pathY == false){
        //path.clear();
        return false;
    }else if ((pathX == true&&pathY == false)||(pathX == false&&pathY == true)){
        path.push_back(Point(x,y));
        return true;
    }else if(pathX == true&&pathY == true){
        path = path_copy;
        path.push_back(Point(x,y));
        findAPath(path,blockedPoints, x-1, y);
        return true;
    }
...
int main(int argc, char* argv[]) {
  ...
  istr = ifstream(argv[1]);
  vector<Point> blockedPoints;
  makeBlockPointVector(istr,blockedPoints);
  std::cout<<"There will be "<< countingSteps(start_x,start_y,blockedPoints)<<" Paths";
  vector<Point> p;
  findAPath(p, blockedPoints, start_x,start_y);
