...
void heapify(std::vector<T> &heap) {
  ...
  // IMPLEMENT THIS FUNCTION (as discussed in lecture)
...
void heap_sort( std::vector<T> & v ) {
  ...
  // FINISH THIS FUNCTION (as discussed in lecture)
  ...
  vector<T> tmpV;
  for (int i = 0; i < s; ++i) {
      tmpV.insert(tmpV.end(),pop_and_return(v));
  }
  v = tmpV;
  // Second draft:  What do you need to modify/rewrite so that you don't need a scratch vector?
