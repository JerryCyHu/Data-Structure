...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    // Implement this function for Lab 12, Checkpoint 2
      ...
      if (p == m_table[index].end()){
          p = m_table[index].insert(m_table[index].end(),key);
          m_size++;
          return make_pair(iterator(this,index,p), true);
      }
      else {
  ...
  iterator begin() {
    ...
    // Implement this function for Lab 12, Checkpoint 3, Part 1
      iterator p(this);
      for (p.m_index = 0; p.m_index < m_table.size(); ++p.m_index) {
          if (!m_table[p.m_index].empty()){
              p.m_list_itr = m_table[p.m_index].begin();
              return p;
          }
      }
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    // Implement this function for Lab 12, Checkpoint 3, Part 2
      vector< list<KeyType> > tmpV = vector<list<KeyType>>(new_size);
      iterator p(this);
      for (p = begin(); p!=end(); ++p) {
          unsigned int hash_value = m_hash(*p);
          unsigned int index = hash_value % tmpV.size();
          tmpV[index].push_back(*p);
      }
