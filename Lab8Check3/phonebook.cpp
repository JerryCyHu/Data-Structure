...
void mapAdd(map<int, string>& phonebook, int number, string const& name){
    phonebook[number] = name;
}

void  mapIdentify(map<int, string>& phonebook, int number){
...
//  identify(phonebook, 4444);
    ...
    mapAdd(phonebook, 1111, "fred");
    mapAdd(phonebook, 2222, "sally");
    mapAdd(phonebook, 3333, "george");

    // test the phonebook
    mapIdentify(phonebook, 2222);
    mapIdentify(phonebook, 4444);
}
